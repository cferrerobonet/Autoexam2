...
class ConfiguracionControlador {
    ...
    private function actualizarConfiguracionSistema() {
        ...
        if (empty($nombre_app)) {
            $_SESSION['error'] = 'El nombre de la aplicación es obligatorio';
            return;
        }
        ...
        if ($this->modelo->actualizarConfiguracionEnv($datos)) {
            ...
            $_SESSION['exito'] = 'Configuración del sistema actualizada correctamente';
        } else {
            $_SESSION['error'] = 'Error al actualizar la configuración del sistema';
        }
    }
    ...
    private function actualizarConfiguracionBaseDatos() {
        ...
        if (empty($host) || empty($nombre) || empty($usuario)) {
            $_SESSION['error'] = 'Todos los campos de base de datos son obligatorios excepto la contraseña';
            return;
        }
        
        // Validar puerto
        if (!is_numeric($puerto) || $puerto < 1 || $puerto > 65535) {
            $_SESSION['error'] = 'El puerto debe ser un número válido';
            return;
        }
        ...
        if (!$validacion['valido']) {
            $_SESSION['error'] = 'Error de conexión: ' . $validacion['mensaje'];
            return;
        }
        ...
        if ($this->modelo->actualizarConfiguracionEnv($datos)) {
            ...
            $_SESSION['exito'] = 'Configuración de base de datos actualizada y verificada correctamente';
        } else {
            $_SESSION['error'] = 'Error al actualizar la configuración de base de datos';
        }
    }
    ...
    private function actualizarConfiguracionCorreo() {
        ...
        if (empty($smtp_host) || empty($smtp_puerto) || empty($smtp_usuario)) {
            $_SESSION['error'] = 'Los campos Host, Puerto y Usuario SMTP son obligatorios';
            return;
        }
        
        // Validar puerto
        if (!is_numeric($smtp_puerto) || $smtp_puerto < 1 || $smtp_puerto > 65535) {
            $_SESSION['error'] = 'El puerto SMTP debe ser un número válido';
            return;
        }
        
        // Validar email
        if (!empty($smtp_from_email) && !filter_var($smtp_from_email, FILTER_VALIDATE_EMAIL)) {
            $_SESSION['error'] = 'El email FROM no es válido';
            return;
        }
        ...
        if (!empty($smtp_contrasena)) {
            ...
            if (!$validacion['valido']) {
                $_SESSION['error'] = 'Error de conexión SMTP: ' . $validacion['mensaje'];
                return;
            }
        }
        ...
        if ($this->modelo->actualizarConfiguracionEnv($datos)) {
            ...
            $_SESSION['exito'] = 'Configuración de correo actualizada correctamente';
        } else {
    ...
    private function actualizarConfiguracionArchivos() {
        ...
        if (!preg_match('/^\d+[KMGT]?B?$/i', $upload_max_size)) {
            $_SESSION['error'] = 'El tamaño máximo debe tener formato válido (ej: 2MB, 10MB)';
            return;
        }
